...
//the '\n' is our end delimiter indicating the end of a complete packet
  ...
  //println(val);
  if (val!=null) {
    String[] steps=split(val.trim(), ':');
    
    if (steps.length==2) {
      player1=int(steps[0]);
      player2=int(steps[1]);
      
      if (!Float.isNaN(player1)) {
        if (player1>200) {
          g_paddle.g_x += PADDLE_SPEED;
          g_paddle.g_x = constrain(g_paddle.g_x, 0, width - PADDLE_WIDTH);
        }
        player1=0;
      }
      if (!Float.isNaN(player2)) {
        if (player2>200) {
          g_paddle.g_x -= PADDLE_SPEED;
          g_paddle.g_x = constrain(g_paddle.g_x, 0, width - PADDLE_WIDTH);
        }
